...
- (void)configureCellWithMessage:(QBChatAbstractMessage *)message
{
    ...
    // Left/Right bubble
    if ([MTLocalStorageService shared].currentUser.ID == message.senderID) {
        ...
        self.dateLabel.text = [NSString stringWithFormat:@"%@, %@", [[MTLocalStorageService shared].currentUser login], time];
